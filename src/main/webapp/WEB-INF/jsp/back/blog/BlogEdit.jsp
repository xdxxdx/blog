									...
									<div class="box-body">
										<div class="form-group">
											...
											<div class="div-layer-left-80">
												<input name='blogTitle' value='${blog.blogTitle}' class='form-control' type='text' placeholder='请输入文章标题'>
											</div>
										</div>
										<div class="form-group">
											...
											<div class="div-layer-left-80">
												<div class="word-count">
													<textarea class="form-control" id="blogAbstract" name="blogAbstract" rows="3" placeholder="摘要,150字以内">${blog.blogAbstract}</textarea>
										...
										<div class="form-group">
										<div class="temp" style="display: none">${blog.blogContent}</div>
										...
										<div class="form-group">
											...
											<div class="div-layer-left-80 check-wrap">
													<c:forEach items="${tagList}" var="tag" varStatus="status">
														...
														<li><input type="checkbox" name="ck_tag" value="${tag.tagId}" <c:if test="${tag.blogId!=0}">checked='checked'</c:if>>${tag.tagName}</li>
											...
											<div class="div-layer-left-80">
												<select class="category" name="categoryId">
													<c:forEach items="${categoryList}" var="category">
														<OPTION value="${category.categoryId}" <c:if test="${blog.categoryId==category.categoryId}">selected</c:if>>${category.categoryName}</OPTION>
													</c:forEach>
												</select>
											</div>
										</div>
										<div class="form-group">
											...
											<div class="div-layer-left-80">
												<c:if test="${blog.isPrivate==1}">
												<input type="checkbox" name="isPrivate" id="isPrivate" checked="checked">
												</c:if>
												<c:if test="${blog.isPrivate==0}">
